class Trace():
    ...
    def Log(db,api_name, trace_id, status, message, request_json, response_json):
        ...
        v_response_json = response_json.replace("\'", '\"').replace('None', 'null').replace('True', 'true').replace(
            'False', 'false')
        query = "call public.log_trace (:p_api_name, :p_status, :p_message, :p_request_json, " \
    ...
    def MakeResponse(db,api_name, trace_id, status, message, request_json, response_json):
        ...
        Trace.Log(db,api_name, trace_id, status, message, request_json, str(response_json))
    ...
    def MakeError(db,api_name, trace_id, status, message, request_json, error_code):
        ...
        Trace.Log(db,api_name, trace_id, status, message, request_json, str(resp))
    ...
    def AddNewTrace(db,api_name, status, request_json):
        """
